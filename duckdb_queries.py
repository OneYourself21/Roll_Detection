...
def select_db(path : str) -> pl.DataFrame:
    ...
    df = duckdb.sql('SELECT * FROM read_parquet($path);', params = {"path" : path}).pl()
    return df
...
def daily_candles(path : str, contract : str, year : int) -> pl.DataFrame:
    ...
    daily_candles_query = """
               ...
               GROUP BY date_trunc('day', ts_event - INTERVAL 18 HOUR) ORDER BY 1;
            """

    df = duckdb.sql(daily_candles_query, params =
    {
        "path" : path,
        "contract" : contract,
...
def daily_returns(path : str, contract : str, year : int) -> pl.DataFrame:
    ...
    daily_returns_query = """ SELECT ts_event,
                       instrument_id,
                       symbol,
    ...
    returns = duckdb.sql(daily_returns_query, params =
    {
        "path" : path,
        "contract" : contract,
        "previous_year" : previous_year,
...
def rolling_stats(path : str, contract : str, year : int, days : int) -> pl.DataFrame:
    ...
    with duckdb.connect() as con:
        ...
        temp_daily_candles_query = """
            CREATE TEMP TABLE daily_candles AS 
            SELECT ts_event,
                   instrument_id,
                   symbol,
        ...
        con.sql(temp_daily_candles_query, params =
        {
            "path": path,
            "contract": contract,
            "previous_year": previous_year,
        ...
        add_atr_query = """WITH RECURSIVE ATR (ts_event, true_range, average_true_range) AS (
        ...
        df = con.sql(add_atr_query, params={"normalising_factor" : normalising_factor, "days" : days}).pl()
